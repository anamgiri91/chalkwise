...
test('concurrent session reads perform one refresh and do not persist access tokens', async () => {
  ...
  const manager = createSessionManager({
    ...
    refresh: async (token) => {
      count++;
      assert.equal(token, 'saved-refresh');
      return session;
    },
  });
  const result = await Promise.all([manager.get(), manager.get(), manager.get()]);
  assert.equal(count, 1);
  assert.equal(result[2]?.accessToken, 'access');
  assert.equal(writes.length, 0);
  await manager.set(session);
...
test('sign-out during refresh cannot resurrect the session', async () => {
  ...
  const refreshing = new Promise<AuthSession>((resolve) => {
    finish = resolve;
  });
  const manager = createSessionManager({
    store: { read: async () => 'old', write: async () => {} },
    refresh: () => refreshing,
  });
...
test('an initial network failure retains the ability to restore a session', async () => {
  let attempts = 0;
  const manager = createSessionManager({
    store: { read: async () => 'saved', write: async () => {} },
    refresh: async () => {
  ...
  assert.equal((await manager.get())?.userId, 'student');
...
test('missing sessions never issue API requests', async () => {
  ...
  await assert.rejects(client('/profile'), { status: 401 });
...
test('API writes propagate stable keys without implicit retries', async () => {
  ...
  const client = createApiClient({
    ...
    fetch: async (url, options) => {
      calls++;
      assert.equal(url, 'https://api.example/v1/lectures');
      assert.equal((options?.headers as Record<string, string>)['Idempotency-Key'], 'stable-id');
      return new Response(
        JSON.stringify({
          error: { code: 'CONFLICT', message: 'Retry safely', requestId: 'trace' },
        }),
        { status: 409 },
      );
    },
  ...
  await assert.rejects(
    client('/lectures', { method: 'POST', body: { title: 'Note' }, idempotencyKey: 'stable-id' }),
    { status: 409, requestId: 'trace' },
  );
