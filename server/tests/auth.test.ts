...
for (const [name, claims] of Object.entries({
  ...
  idToken: { token_use: 'id' },
  invalidSubject: { sub: 'admin' },
  missingExpiry: { exp: undefined },
})) {
  test(`rejects ${name}`, async () => {
    await assert.rejects(verifyCognitoToken(await token(claims), keys, issuer, 'client'), {
      statusCode: 401,
    });
  });
}
test('rejects unsigned and tampered tokens', async () => {
  ...
  await assert.rejects(verifyCognitoToken(parts.join('.'), keys, issuer, 'client'), {
    statusCode: 401,
  });
});
