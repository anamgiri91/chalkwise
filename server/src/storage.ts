...
export function validatePhoto(data: string, mimeType: string): Buffer {
  ...
  if (!matches)
    throw new ApiError(400, 'INVALID_PHOTO', 'The photo format does not match its content type.');
  return bytes;
}
  ...
  return {
      ...
      try {
        await client.send(
          new PutObjectCommand({
            ...
            IfNoneMatch: '*',
            Metadata: { sha256: hash(photo.bytes) },
        ...
        const existing = await client
          .send(new HeadObjectCommand({ Bucket: bucket, Key: path }), options())
          .catch(() => null);
        if (
          existing?.Metadata?.sha256 !== hash(photo.bytes) ||
          existing.ContentLength !== photo.bytes.length
        )
          throw error;
      }
    },
    async read(path) {
      ...
      if (bytes.length > MAX_PHOTO_BYTES)
        throw new ApiError(422, 'INVALID_PHOTO', 'The original is too large.');
