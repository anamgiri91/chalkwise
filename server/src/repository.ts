...
const many = async (db: PoolClient, sql: string, values: unknown[] = []) =>
...
export function repository(database: Database, storage: Storage) {
  const run = <T>(user: string, work: (db: PoolClient) => Promise<T>) =>
    database.asUser(user, work);
  return {
    ...
    searchProfiles: (user: string, query: string) =>
      run(user, (db) =>
        many(
          db,
    ...
    courses: (user: string, enrolled = false) =>
      run(user, (db) =>
        many(
          db,
    ...
    lectures: (user: string, courseId: string) =>
      run(user, (db) =>
        many(
          db,
      ...
      run(user, async (db) => {
        ...
        if (
          ...
          throw new ApiError(
            ...
            'This save key was already used for different notes.',
          );
        }
        return saved;
      }),
    ...
    setSharing: (user: string, id: string, shared: boolean) =>
      run(user, async (db) =>
        found(
          await one(
            db,
    ...
    requestFriend: (user: string, addressee: string) =>
      run(user, async (db) => {
        if (user === addressee)
    ...
    acceptFriend: (user: string, id: string) =>
      run(user, async (db) =>
        found(
          await one(
            db,
    ...
    materials: (user: string, lectureId: string) =>
      run(user, (db) =>
        many(
          db,
    ...
    upload: async (user: string, input: z.infer<typeof uploadInput>) => {
      ...
      if (row.sha256 !== digest || row.mime_type !== input.mimeType)
        throw new ApiError(
          409,
          'IDEMPOTENCY_CONFLICT',
          'This upload key belongs to another photo.',
        );
      ...
      run(user, async (db) => {
        ...
        if (!row)
          throw new ApiError(
            409,
            'ATTACHMENT_CONFLICT',
            'Photo is unavailable or already attached.',
          );
        return row;
      }),
    ...
    copyLecture: async (user: string, sourceId: string) => {
      ...
      if (source.photos.length > 6)
        throw new ApiError(422, 'TOO_MANY_PHOTOS', 'This notebook has too many originals to copy.');
