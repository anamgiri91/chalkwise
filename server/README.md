...
2. Provision a PostgreSQL database and distinct migration/runtime identities. Review `infra/runtime-role.sql`, then `migrations/001_initial.sql` and `migrations/002_review_events.sql`, in order. Apply them only after explicit approval; startup never applies migrations. Grant `chalkwise_app` to the runtime login, never the migration role. The API refuses a superuser, BYPASSRLS login or table-owner membership.
