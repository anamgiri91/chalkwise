...
const {parseAskLectureResult}=require('../../../src/lib/askLecture.ts');
...
function request(body={lectureId:id,question:'What are the deletion cases?'},key='test-key',method='POST'){
...
(async()=>{
 const cases=[
  ...
  ['empty question',{body:{lectureId:id,question:' '}},400,0],['long question',{body:{lectureId:id,question:'x'.repeat(2001)}},400,0],
  ...
  ['invalid answer',{answer:{answer:''}},502,4],['blocked',{finish:'SAFETY'},502,4],
 ...
 for(const [name,o,status,count] of cases){
  let calls=0;const fetcher=async(url,init)=>{
   ...
   assert.match(body.systemInstruction.parts[0].text,/ONLY the supplied/);assert.match(body.systemInstruction.parts[0].text,/not found in this lecture/);
   ...
   return o.status?new Response('sensitive',{status:o.status}):json({candidates:[{finishReason:o.finish??'STOP',content:{parts:[{text:JSON.stringify(o.answer??{answer:' Leaf, one child, two children. '})}]}}]});
  };
  ...
  if(status===200)assert.equal(JSON.parse(text).answer,'Leaf, one child, two children.');
 }
 ...
 mode='mock';await assert.rejects(exports.askLecture(id,'Question'),/supabase/);await assert.rejects(exports.askLecture(id,' '),/required/);assert.equal(calls,3);
 console.log(`PASS: handler typecheck, ${cases.length} Q&A scenarios, timeout and service validation. Original photo bytes and lecture fields included; no network.`);
