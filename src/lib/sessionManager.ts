export type AuthSession = { accessToken: string; refreshToken: string; userId: string; expiresAt: number };
export type RefreshStore = { read(): Promise<string | null>; write(value: string | null): Promise<void> };
...
export function createSessionManager(deps: {
  ...
  return {
    subscribe(listener:(id:string|null)=>void) { listeners.add(listener); return ()=>{ listeners.delete(listener); }; },
    async set(next:AuthSession) {
      const version=++generation;
      await persist(next.refreshToken);
      if (version!==generation) return;
      session=next; loaded=true; notify();
    },
    async clear() {
      generation++; session=null; loaded=true; pending=null; notify();
      await persist(null);
    },
    async get():Promise<AuthSession|null> {
      ...
      const current=(async()=>{
        ...
        if (!refreshToken) { loaded=true; return null; }
        const refreshed=await deps.refresh(refreshToken);
        if (version!==generation) return null;
        session=refreshed; loaded=true; notify(); return refreshed;
      })();
      pending=current;
