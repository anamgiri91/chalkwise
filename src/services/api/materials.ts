...
export async function uploadApiMaterial(input:MaterialUploadInput):Promise<Material> {
  if(input.type!=='photo' || !input.uri.startsWith('file://')) throw new Error('Choose a local photo to upload.');
  ...
  const task=(async()=>{
    ...
    if(!file.exists || file.size<=0 || file.size>10*1024*1024) throw new Error('Choose a photo no larger than 10 MiB.');
    const data=await file.base64();
    return apiRequest<Material>('/materials',{ method:'POST',body:{ id,mimeType:input.mimeType,data } });
  })();
