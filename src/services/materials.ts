...
export async function uploadMaterial(input: MaterialUploadInput): Promise<Material> {
  ...
  if (getDataMode() !== 'supabase') {
    throw new Error(
      'Photo upload requires EXPO_PUBLIC_DATA_MODE=supabase. Mock uploads are not implemented.',
    );
  }
  ...
  if (!input.uri.startsWith('file://'))
    throw new Error('Photo upload requires a local file:// URI.');
  ...
  try {
    ...
    if (error) cleanupResult = `Cleanup failed: ${error.message}`;
      ...
      cleanupResult =
        'Cleanup did not confirm removal; the object may be registered or require manual cleanup.';
...
export async function attachMaterialToLecture(
  ...
  if (getDataMode() === 'api')
    return apiRequest(`/materials/${encodeURIComponent(materialId)}/lecture`, {
      method: 'PUT',
      body: { lectureId },
    });
  if (getDataMode() !== 'supabase')
    throw new Error('Material attachment requires EXPO_PUBLIC_DATA_MODE=supabase.');
  if (!materialId.trim() || !lectureId.trim())
    throw new Error('Material and lecture IDs are required.');
  const { supabase } = await import('@/lib/supabase');
  const { data, error } = await supabase
    .from('materials')
...
export async function getMaterialUrl(
  ...
  if (getDataMode() === 'api') {
    try {
      return (
        await apiRequest<{ url: string }>(`/materials/${encodeURIComponent(material.id)}/url`)
      ).url;
    } catch {
      return null;
    }
...
export async function copyLectureMaterials(sourceId: string, targetId: string): Promise<void> {
  if (getDataMode() === 'api')
    throw new Error(
      'Copy the notebook through copyLectureToMyNotes so originals are saved together.',
    );
  ...
  for (const original of originals) {
    ...
    if (!extension || !Object.values(extensions).includes(extension))
      throw new Error('Unsupported source photo format.');
    const storagePath = `materials/${id}/photo.${extension}`;
    const read = () =>
      supabase
        .from('materials')
    ...
    if (!row) {
      ...
      if (!row)
        throw new Error(
          `Could not register copied photo: ${insertError?.message ?? result.error?.message ?? 'Try again.'}`,
        );
    }
    if (
      row.storage_path !== storagePath ||
      (row.lecture_id !== null && row.lecture_id !== targetId)
    ) {
