...
2. **Schema review.** Review `server/infra/runtime-role.sql`, `server/migrations/001_initial.sql`, `002_review_events.sql` and `003_study_features.sql`, in order. Apply them only to the new database after explicit approval. Grant the runtime login `chalkwise_app`, without superuser, BYPASSRLS, schema ownership, or membership in the migration role. Exercise the isolation suite in a disposable database first.
